...
export default class FixedList extends AbstractList {
  initializeProperties = () => {
    const { rowHeight, column, rowColumns, data, height, offScreenRow } = this.props;

    // Validate

    if (rowColumns) {
      if (rowColumns.reduce((acc, current) => acc + current, 0) !== data.length) {
        throw Error(
    ...
    const rowToDataIndexMap = mapRowIndexToDataIndex(calculatedRowColumns, data.length);
    ...
    let totalNumOfRenderedRows = numOfVisibleRow + numOfInvisibleRowOnEachDirection * 2;
  ...
  constructor(props) {
    ...
    const initialRenderedRowIndex = this.initialArrayTemplate.map((_, index) => index);
  ...
  shouldResetList = (prevProps) => {
    const currentProp = this.props;
    if (prevProps === currentProp) return false;
    const { rowHeight, column, rowColumns, height, data, offScreenRow } = currentProp;
  ...
  getTopViewportRowIndex = (scrollTop) => {
    return Math.floor(scrollTop / this.props.rowHeight);
  };

  getBottomViewportRowIndex = (viewportBottom) => {
